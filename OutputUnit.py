...
class OutputUnit(object):
    def __init__(self, input_size, output_size, scope_name):
        ...
        with tf.compat.v1.variable_scope(scope_name):
            self.W = tf.compat.v1.get_variable('W', [input_size, output_size])
            self.b = tf.compat.v1.get_variable('b', [output_size])

        self.params.update({'W': self.W, 'b': self.b})

    def __call__(self, x, finished = None):
        out = tf.compat.v1.nn.xw_plus_b(x, self.W, self.b)

        if finished is not None:
            out = tf.compat.v1.where(finished, tf.compat.v1.zeros_like(out), out)
            #out = tf.compat.v1.multiply(1 - finished, out)
        return out
